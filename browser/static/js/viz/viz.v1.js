!function(){
  ...
  function ch(_){
    ...
    _.each(function() {
	  ...
	  var labels = grps.insert("text","path + *").attr("class","label");

	  labels.filter(function(d){ return d.type=="g"})
		.style("text-anchor",function(d){var a =angle(d); return a < π2 || a>τ-π2 ? "start" : "end";})
        .each(function(d) { this._current = d; })
		.insert("tspan").text(function(d){ return d.source;}).style("text-anchor","middle")
	    .attr("x",function(d){ return r*Math.cos(angle(d));})
	    .attr("y",function(d){ return r*Math.sin(angle(d));})
		;
		
	  labels.filter(function(d){ return d.type=="g"})
		.insert("tspan").text(function(d){ return '('+d.value.toLocaleString()+')';}).style("text-anchor","middle")
	    .attr("x",function(d){ return r*Math.cos(angle(d));})
	    .attr("y",function(d){ return r*Math.sin(angle(d))+15;});
  ...
  function transition(f){
    ...
    function labelTweenyVal(a) {
      var i = d3.interpolate(this._current, a);
      this._current = i(0);
      return function(t) { return r*Math.sin(angle(i(t))) + 15; };
    }  
	...
	gp.select("tspan:first-child").text(function(d){ return d.source;}).transition().duration(duration)
	    .attrTween("x",labelTweenx)
	    .attrTween("y",labelTweeny);
	gp.select("tspan:last-child").text(function(d){ return '('+d.value.toLocaleString()+')';}).transition().duration(duration)
	    .attrTween("x",labelTweenx)
	    .attrTween("y",labelTweenyVal);
  }
  function angle(d){
